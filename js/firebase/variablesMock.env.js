// Se dejan los datos de autenticación de Firebase para validación de la entrega. 
// Se realizó corrección para encriptar la clave y evitar exposición directa en el código fuente.
// ToDo: Almacenar las variables de entorno en un archivo .env real 

/**
 * @description Función que aloja y retorna los datos de autenticación de Firebase
 * Se dejan los datos de autenticación de para validación de la entrega. 
 * ToDo: Almacenar las variables de entorno en un archivo .env real para evitar la carga y divulgación de información sensible (riesgos críticos de seguridad)
 * @returns Las variables con los datos de acceso a Firebase
 */

export async function obtenerAuthVariables(decryptKey) {

    const PROJECT_ID = "gestionhc-2144";
    const encryptedServiceAccountKey = '';
    const MD5_HASH_KEY = 'de98e5765f6a546f7262f02c8bfecc00';

    if (!decryptKey) {
        throw new Error("Se requiere la clave de descifrado");
    }

    if (typeof CryptoJS === 'undefined') {
        throw new Error("CryptoJS no está disponible. Asegúrese de incluir la librería CryptoJS antes de usar obtenerAuthVariables.");
    }

    try {
        const bytes = CryptoJS.AES.decrypt(encryptedServiceAccountKey, decryptKey);
        const decryptedServiceAccountKey = bytes.toString(CryptoJS.enc.Utf8);

        // Si el descifrado con clave incorrecta devuelve cadena vacía o no-UTF8
        if (!decryptedServiceAccountKey || decryptedServiceAccountKey.trim() === '') {
            throw new Error('La clave de cifrado es incorrecta o el resultado del descifrado es vacío');
        }

        let SERVICE_ACCOUNT_KEY_JSON;
        try {
            SERVICE_ACCOUNT_KEY_JSON = JSON.parse(decryptedServiceAccountKey);
        } catch (parseError) {
            // Si el contenido descifrado no es un JSON válido muy probablemente se debe a una clave incorrecta
            throw new Error('El contenido descifrado no es un JSON válido. ¿Valide la clave de de descifrado?');
        }

        const userInputHash = CryptoJS.MD5(JSON.stringify(SERVICE_ACCOUNT_KEY_JSON)).toString();
        if (userInputHash === MD5_HASH_KEY) {
            return {
                PROJECT_ID,
                SERVICE_ACCOUNT_KEY_JSON,
            };
        } else {
            throw new Error('Clave de cifrado incorrecta (el hash no coincide)');
        }
    } catch (error) {
        const msg = (error && error.message) ? error.message : String(error);
        // Normalizar casos en que CryptoJS arroja "Malformed UTF-8 data" u otros errores de codificación
        if (/malformed utf-8/i.test(msg) || /malformed.*utf-8/i.test(msg) || /utf-8.*malformed/i.test(msg)) {
            throw new Error('Clave de cifrado incorrecta (Los datos descifrados no son UTF-8 válidos)');
        }
        throw new Error('Error al descifrar las variables de autenticación: ' + msg);
    }
}


